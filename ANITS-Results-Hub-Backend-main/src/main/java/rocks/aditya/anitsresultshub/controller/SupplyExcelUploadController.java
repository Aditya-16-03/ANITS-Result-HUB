...
import rocks.aditya.anitsresultshub.service.SupplyExcelUploadService;
...
public class SupplyExcelUploadController {
    ...
    public ResponseEntity<String> uploadSupply(
        ...
        // âœ… Supply only has 1 header row (index 0)
        ...
        log.info("ðŸ“Š Preparing to create supply table: {}", tableName);
        log.info("ðŸ“ Final columns: {}", columns);

        supplyUploadService.uploadExcel(file.getInputStream(), tableName, columns);

        return ResponseEntity.ok("âœ… Supplementary Results uploaded into table: " + tableName);
    }
}